...
class OpenSupCon(Algo):
    ...
    def indexing_mask_factory(self, cosine_dist, target, label_concat, logit, target_unlabeled=None):
        ...
        ood_pred_mask_oracle = (torch.cat([target, target_unlabeled]) >= self.args.no_seen).repeat(2)

        ############ OOD assignment by fake label
        ood_pred_mask_naive = unlabeled_ind_mask * (label_concat >= self.args.no_seen).repeat(2)
    ...
    def forward_cifar(self, x, x2, target, evalmode=False, target_unlabeled=None):
        ...
        if not evalmode:
            ...
            self.update_prototype_lazy(feat_proj[updated_ind][rand_order], label_concat[updated_ind][rand_order], momemt=0.9)
